...
class ConvoChatWebhooksService
{
    ...
    public const WEBHOOKS_ENDPOINT = '/api/webhooks';
    public const WEBHOOK_DELETE_ENDPOINT = '/api/webhooks/{id}';
    ...
    public function getWebhooks(array $filters = []): array
    {
        ...
        return $this->makeRequest(self::WEBHOOKS_ENDPOINT, $data, 'GET');
    }

    public function createWebhook(array $webhookData): array
    {
        $requiredParams = ['url', 'events'];
        $this->validateRequiredParams($webhookData, $requiredParams);

        $data = array_merge([
            'secret' => $this->apiKey,
        ], $webhookData);

        return $this->makeRequest(self::WEBHOOKS_ENDPOINT, $data);
    }

    public function deleteWebhook(string $webhookId): array
    {
        $endpoint = str_replace('{id}', $webhookId, self::WEBHOOK_DELETE_ENDPOINT);
    ...
    protected function makeRequest(string $endpoint, array $data, string $method = 'POST'): array
    {
        try {
            ...
            if (config('convochat.log_requests', false)) {
                Log::info('ConvoChat Webhooks API Request Success', [
                    'endpoint' => $endpoint,
                    'method' => $method,
            ...
            Log::error('ConvoChat Webhooks API Error', [
                'endpoint' => $endpoint,
                'method' => $method,
            ...
            throw new \Exception("ConvoChat Webhooks API Error: " . $e->getMessage());
        }
    }
