...
class ConvoChatListsService
{
    ...
    public const LISTS_ENDPOINT = '/api/lists';
    public const LIST_DETAIL_ENDPOINT = '/api/lists/{id}';
    ...
    public function getLists(array $filters = []): array
    {
        $data = array_merge([
            'secret' => $this->apiKey,
        ], $filters);

        return $this->makeRequest(self::LISTS_ENDPOINT, $data, 'GET');
    }

    public function createList(array $listData): array
    {
        $requiredParams = ['name'];
        $this->validateRequiredParams($listData, $requiredParams);

        $data = array_merge([
            'secret' => $this->apiKey,
        ], $listData);

        return $this->makeRequest(self::LISTS_ENDPOINT, $data);
    }

    public function getList(string $listId): array
    {
        $endpoint = str_replace('{id}', $listId, self::LIST_DETAIL_ENDPOINT);
    ...
    protected function makeRequest(string $endpoint, array $data, string $method = 'POST'): array
    {
        try {
            ...
            if (config('convochat.log_requests', false)) {
                Log::info('ConvoChat Lists API Request Success', [
                    'endpoint' => $endpoint,
                    'method' => $method,
            ...
            Log::error('ConvoChat Lists API Error', [
                'endpoint' => $endpoint,
                'method' => $method,
            ...
            throw new \Exception("ConvoChat Lists API Error: " . $e->getMessage());
        }
    }
