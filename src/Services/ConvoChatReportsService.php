...
class ConvoChatReportsService
{
    ...
    public const REPORTS_SMS_ENDPOINT = '/api/reports/sms';
    public const REPORTS_WHATSAPP_ENDPOINT = '/api/reports/whatsapp';
    public const REPORTS_CAMPAIGNS_ENDPOINT = '/api/reports/campaigns';
    ...
    public function getSmsReport(array $filters = []): array
    {
        ...
        return $this->makeRequest(self::REPORTS_SMS_ENDPOINT, $data, 'GET');
    }

    public function getWhatsAppReport(array $filters = []): array
    {
        $data = array_merge([
            'secret' => $this->apiKey,
        ], $filters);

        return $this->makeRequest(self::REPORTS_WHATSAPP_ENDPOINT, $data, 'GET');
    }

    public function getCampaignsReport(array $filters = []): array
    {
        $data = array_merge([
            'secret' => $this->apiKey,
        ], $filters);

        return $this->makeRequest(self::REPORTS_CAMPAIGNS_ENDPOINT, $data, 'GET');
    }
    ...
    protected function makeRequest(string $endpoint, array $data, string $method = 'POST'): array
    {
        try {
            ...
            if (config('convochat.log_requests', false)) {
                Log::info('ConvoChat Reports API Request Success', [
                    'endpoint' => $endpoint,
                    'method' => $method,
            ...
            Log::error('ConvoChat Reports API Error', [
                'endpoint' => $endpoint,
                'method' => $method,
            ...
            throw new \Exception("ConvoChat Reports API Error: " . $e->getMessage());
        }
    }
